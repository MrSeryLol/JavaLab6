...
public class TextObject extends Observable implements Runnable{
    ...
    public TextObject (Color col, String text, int speed, String numObj, Application window) {
        xplus = true; yplus = true;
        x = 0; y = 30;
        this._text = text;
        this._speed = speed;
        this.col = col;
        this._numObj = Integer.parseInt(numObj);
        this._window = window;
    ...
    public void run(){
        while (true){
            ...
            if(xplus) x += _speed; else x -= _speed;
            if(yplus) y += _speed; else y -= _speed;
